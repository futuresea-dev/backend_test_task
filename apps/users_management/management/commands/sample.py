...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        Denys_user = self.create_superuser(
            username="Denys",
            password="1516",
        ...
        try:
            ...
            Participant.objects.create(user=Denys_user, thread=thread2)
            Participant.objects.create(user=user2, thread=thread2)

            # Create messages for thread 2
            Message.objects.create(thread=thread2, sender=Denys_user, text="Hi from Denys!")
            Message.objects.create(thread=thread2, sender=user2, text="Hi from user2!")
            Message.objects.create(thread=thread2, sender=Denys_user, text="How's your day?")
            Message.objects.create(thread=thread2, sender=user2, text="It's been productive, what about yours?")
            Message.objects.create(thread=thread2, sender=Denys_user, text="Busy, but good!")
